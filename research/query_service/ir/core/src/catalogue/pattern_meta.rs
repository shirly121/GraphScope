...
impl PatternMeta {
    ...
    pub fn get_all_vertex_label_ids(&self) -> impl Iterator<Item = &PatternLabelId> {
        self.vertex_label_map
            .iter()
            .map(|(_, vertex_id)| vertex_id)
    }

    pub fn get_all_edge_label_ids(&self) -> impl Iterator<Item = &PatternLabelId> {
        self.edge_label_map
            .iter()
    ...
    pub fn get_vertex_label_id(&self, label_name: &str) -> Option<&PatternLabelId> {
        self.vertex_label_map.get(label_name)
    }

    pub fn get_edge_label_id(&self, label_name: &str) -> Option<&PatternLabelId> {
        self.edge_label_map.get(label_name)
    }

    /// Given a soruce vertex label, find all its neighboring adjacent vertices(label)
    pub fn get_adjacent_vlabels<'a>(
        &'a self, src_v_label: PatternLabelId,
    ) -> Box<dyn Iterator<Item = &'a PatternLabelId> + 'a> {
    ...
    pub fn get_adjacent_elabels<'a>(
        &'a self, src_v_label: PatternLabelId,
    ) -> Box<dyn Iterator<Item = &'a (PatternLabelId, PatternDirection)> + 'a> {
        match self.v2edges_meta.get(&src_v_label) {
            Some(connections) => Box::new(connections.iter()),
            None => Box::new(std::iter::empty()),
        }
    }

    /// Given a source edge label, find all possible pairs of its (src vertex label, dst vertex label)
    pub fn get_associated_vlabels<'a>(
        &'a self, src_e_label: PatternLabelId,
    ) -> Box<dyn Iterator<Item = &'a (PatternLabelId, PatternLabelId)> + 'a> {
    ...
    pub fn get_associated_elabels<'a>(
        &'a self, src_v_label: PatternLabelId, dst_v_label: PatternLabelId,
    ) -> Box<dyn Iterator<Item = &'a (PatternLabelId, PatternDirection)> + 'a> {
        match self
            .vv2edges_meta
