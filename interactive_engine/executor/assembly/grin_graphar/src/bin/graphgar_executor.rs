...
use grin_graphar::error::{StartServerError, StartServerResult};
use log::info;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        ...
        //let parent_dir = "workspaces/GraphScope/gstest/modern_graph_graphar";
        let uri = format!(
            "graphar:///workspaces/GraphScope/gstest/modern_graph_graphar/modern_graph/modern_graph.graph.yml?partition_num=1&strategy=hash"       
        );
        println!("uri: {:?}", uri);
