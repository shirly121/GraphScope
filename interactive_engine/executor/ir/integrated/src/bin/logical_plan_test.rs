...
use ir_common::KeyId;
use ir_core::catalogue::extend_step::{DefiniteExtendEdge, DefiniteExtendStep};
use ir_core::catalogue::pattern::PatternVertex;
use ir_core::catalogue::plan::build_distributed_match_plan;
use ir_core::catalogue::plan::build_stand_alone_match_plan;
...
use runtime_integration::{read_pattern, read_pattern_meta, InitializeJobAssembly, QueryExpGraph};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut pb_plan = if config.is_distributed {
        build_distributed_match_plan(&pattern, define_definite_extend_steps(), &pattern_meta).unwrap()
    } else {
        build_stand_alone_match_plan(&pattern, define_definite_extend_steps(), &pattern_meta).unwrap()
    };
...
fn define_definite_extend_steps() -> Vec<DefiniteExtendStep> {
