...
use crate::process::entry::{DynEntry, Entry, IDEntry};
use crate::process::operator::map::FilterMapFuncGen;
use crate::process::record::Record;

/// An ExpandVertexOrIntersect (or ExpandEdgeVOrIntersect) operator to expand neighbors
...
pub struct Intersection {
    // TODO: Notice that if only id saved, will occur bugs when filter vertices by label after intersection.
    vertex_vec: Vec<IDEntry>,
    count_vec: Vec<u32>,
}
...
impl Intersection {
    pub fn from_iter<I: Iterator<Item = ID>>(iter: I) -> Intersection {
        let mut vertex_count_map = BTreeMap::new();
        for vid in iter {
            let cnt = vertex_count_map
                .entry(IDEntry { id: vid })
                .or_insert(0);
            *cnt += 1;
        }
    ...
    pub fn drain(&mut self) -> impl Iterator<Item = IDEntry> + '_ {
        self.vertex_vec
            .drain(..)
            .zip(&self.count_vec)
            .flat_map(move |(vertex, count)| std::iter::repeat(vertex).take(*count as usize))
...
impl Decode for Intersection {
    fn read_from<R: ReadExt>(reader: &mut R) -> std::io::Result<Self> {
        let vertex_vec = <Vec<IDEntry>>::read_from(reader)?;
    ...
    fn exec(&self, mut input: Record) -> FnResult<Option<Record>> {
        ...
        if let Some(id) = entry.as_vid() {
            let iter = self.stmt.exec(id)?.map(|e| {
                if let Some(v) = e.as_vertex() {
                    v.id()
                } else {
                    let e = e.as_edge().unwrap();
                    e.get_other_id()
                }
            });
