...
impl PlanPath {
    ...
    pub fn to_match_plan(
        ...
        if let PlanStep::Extend(src_extend) = self
            .plan_steps
            .pop_front()
            .ok_or(IrError::Unsupported("Cannot generate plan for empty plan path".to_string()))?
        {
            append_start_nodes_to_pb_plan(&mut match_plan, src_extend, origin_pattern)?;
        } else {
            return Err(IrError::Unsupported("The src plan step has to be extend step".to_string()));
        }
...
impl Pattern {
    ...
    pub fn generate_simple_extend_match_plan(
        ...
        let mut pb_plan = plan_path.to_match_plan(is_distributed, self, pattern_meta)?;
    ...
    pub fn generate_optimized_match_plan(
        ...
        plan_path
            .to_match_plan(is_distributed, self, pattern_meta)
            .map(|mut plan| {
                pb_plan_add_source_operator(&mut plan);
                pb_plan_add_count_sink_operator(&mut plan);
                plan
            })
    }
}
...
pub fn get_plan_path(pattern: Pattern, catalog: &mut Catalogue) -> (PlanPath, CostCount) {
    ...
    if let Some(pattern_index) = catalog.get_pattern_index(&pattern_code) {
        ...
        for (sub_pattern, mut extend_step) in sub_patterns_extend_steps {
            ...
            let (mut extend_steps, pre_cost) = get_plan_path(sub_pattern, catalog);
            ...
            {
                extend_steps.add_plan_step_back(extend_step.into());
                optimal_plan_path = extend_steps;
...
fn get_plan_path_in_catalog(
    ...
    if pattern.get_vertices_num() == 1 {
        ...
        let mut optimal_extend_steps = PlanPath::default();
        let mut min_cost = CostCount::max_value();
        let mut max_predicate_num = usize::MIN;
        ...
        for approach in approaches {
            ...
            extend_steps.add_plan_step_back(definite_extend_step);
...
fn pb_plan_add_source_operator(pb_plan: &mut pb::LogicalPlan) -> Option<()> {
    let source = pb::Scan {
        scan_opt: 0,
        alias: None,
    ...
    insert_node_to_pb_plan(
        pb_plan,
        0,
        pb::logical_plan::Node { opr: Some(source.into()), children: vec![1] },
    )
    .ok()
}
...
// fn remove_node_from_pb_plan(pb_plan: &mut pb::LogicalPlan, index: usize) -> IrResult<()> {
//     if pb_plan.nodes.len() < index {
//         return Err(IrError::Unsupported(
//             "Failed in Plan Generation: Node Insertion, Index out of bound".to_string(),
//         ));
