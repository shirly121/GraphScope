...
/// Replace "Place" "Organisation" in dir_name
fn replace_special_tag(dir_name: &String) -> String {
    dir_name
        .replace("City", "Place")
...
fn main() {
    ...
    for _entry in read_dir(&ldbc_data_dir).unwrap() {
        ...
        let dirname = entry.file_name().to_str().unwrap().to_string();
        for _sceond_entry in read_dir(ldbc_data_dir.join(dirname)).unwrap() {
            let second_entry = _sceond_entry.unwrap();
            let fname = second_entry
                .file_name()
                .to_str()
                .unwrap()
                .to_string();
            // start with '.'
            let label_fname = replace_special_tag(&fname);
            if !is_hidden_file(&fname) {
                if is_vertex_file(&fname) {
                    vertex_dirs.push((label_fname.to_uppercase(), second_entry.path()));
                } else {
                    edge_dirs.push((label_fname.to_uppercase(), second_entry.path()));
                }
            }
        }
    }
    ...
    timely::execute_from_args(timely_args.into_iter(), move |worker| {
        ...
        if (is_whole_data && index < peers / num_machines) | !is_whole_data {
            ...
            for (vertex_type, vertex_dir) in &vertex_dirs {
                ...
                let mut count = 0;
                for _entry in read_dir(vertex_dir).unwrap() {
                    let entry = _entry.unwrap();
                    ...
                    let mut split = fname.split("-").collect::<Vec<&str>>();
                    let part_index = split[1].parse::<usize>().unwrap();
                    if part_index % num_machines != index {
                        continue;
                    }

                    let _file = File::open(vertex_dir.join(&fname));
                    if _file.is_err() {
                        // Log and skip processing current file
                        error!("Open {:?} error - {:?}", vertex_dir.join(&fname), _file);
                        continue;
                    }
            ...
            for (edge_type, edge_dir) in &edge_dirs {
                ...
                let mut count = 0;
                for _entry in read_dir(edge_dir).unwrap() {
                    let entry = _entry.unwrap();
                    let fname = entry.file_name().to_str().unwrap().to_string();
                    if !fname.ends_with(".csv") {
                        continue;
                    ...
                    println!("Read file {}", fname);
                    let _file = File::open(edge_dir.join(&fname));
                    if _file.is_err() {
                        // Log and skip processing current file
                        error!("Open {:?} error - {:?}", edge_dir.join(&fname), _file);
                        continue;
                    }
                    let reader = BufReader::new(_file.unwrap());
                    for _line in reader.lines() {
