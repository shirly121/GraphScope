...
fn main() {
    ...
    println!("raw_dir {}, graph_dir {}, schema_f {}", raw_dir, graph_dir, schema_f);

    let handle = std::thread::spawn(move || {
        let mut loader: GraphLoader =
            GraphLoader::new(raw_dir, graph_dir, schema_f, 20, partition_index, partition_num);
    ...
    println!("Test opening graph with partition {}", partition_index);
