...
fn main() {
    ...
    if t == 0 {
        ...
        let mut csr = SingleCsr::<InternalId>::new();
        csr = import::<SingleCsr<InternalId>, _>(graph_data_path.clone()).unwrap();

        csr.serialize(&output_path);

        let now = Instant::now();
        let mut new_csr1 = SingleCsr::<InternalId>::new();
        new_csr1 = import::<SingleCsr<InternalId>, _>(graph_data_path.clone()).unwrap();
        ...
        let mut csr = MutableCsr::<InternalId>::new();
        csr = import::<MutableCsr<InternalId>, _>(graph_data_path.clone()).unwrap();

        csr.serialize(&output_path);

        let now = Instant::now();
        let mut new_csr1 = MutableCsr::<InternalId>::new();
        new_csr1 = import::<MutableCsr<InternalId>, _>(graph_data_path.clone()).unwrap();
        ...
        let now = Instant::now();
        let mut table1 = ColTable::import(graph_data_path.clone()).unwrap();
