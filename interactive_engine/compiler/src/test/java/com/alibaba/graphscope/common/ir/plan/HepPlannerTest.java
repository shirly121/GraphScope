...
public class HepPlannerTest {
    ...
    public void push_filter_2_test() {
        ...
        // Utils.mockPlanner(FilterMatchRule.Config.DEFAULT.toRule());
        //        planner.setRoot(filter);
        //        RelNode after = planner.findBestExp();
        //        Assert.assertEquals("GraphLogicalSingleMatch(input=[null],
        // sentence=[GraphLogicalExpand(tableConfig=[{isAll=false, tables=[knows]}], alias=[y],
        // fusedFilter=[[=(DEFAULT.weight, 1.0E0)]], opt=[OUT])\n" +
        //                "  GraphLogicalSource(tableConfig=[{isAll=false, tables=[person]}],
        // alias=[x], opt=[VERTEX])\n" +
    ...
    public void push_filter_4_test() {
        ...
        RelNode after = planner.findBestExp();
        Assert.assertEquals(
                "LogicalFilter(condition=[OR(=(x.age, 10), =(x.age, 20))])\n"
                    + "  GraphLogicalSingleMatch(input=[null],"
                    + " sentence=[GraphLogicalExpand(tableConfig=[{isAll=false, tables=[knows]}],"
                    + " alias=[y], opt=[OUT])\n"
                    + "  GraphLogicalSource(tableConfig=[{isAll=false, tables=[person]}],"
    ...
    public void test() {
        Configs configs =
                new Configs(
                        ImmutableMap.of(
                                "graph.planner",
                                "{\"isOn\":true,\"opt\":\"CBO\",\"rules\":[\"FilterMatchRule\"]}"));
