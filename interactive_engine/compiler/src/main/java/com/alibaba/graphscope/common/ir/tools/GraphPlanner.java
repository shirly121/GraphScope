...
public class GraphPlanner {
    private final static Logger logger = LoggerFactory.getLogger(GraphPlanner.class);
    ...
    private RelOptPlanner createRelOptPlanner(PlannerConfig plannerConfig) {
        if (plannerConfig.isOn()) {
            RelBuilderFactory relBuilderFactory = (RelOptCluster cluster, @Nullable RelOptSchema schema) ->
                    GraphBuilder.create(
                                    ...
                                    k -> {
                                        if (k.equals(
                                            ...
                                            hepBuilder.addRuleInstance(
                                                    CoreRules.FILTER_INTO_JOIN.config.withRelBuilderFactory(relBuilderFactory).toRule());
                                            logger.info("add FILTER_INTO_JOIN");
                                        } else if (k.equals(
                                                FilterMatchRule.class.getSimpleName())) {
                                            hepBuilder.addRuleInstance(
                                                    FilterMatchRule.Config.DEFAULT.withRelBuilderFactory(relBuilderFactory).toRule());
                                            logger.info("add FilterMatchRule");
                                        } else if (k.equals(
                                                NotExistToAntiJoinRule.class.getSimpleName())) {
                                            hepBuilder.addRuleInstance(
                                                    NotExistToAntiJoinRule.Config.DEFAULT.withRelBuilderFactory(relBuilderFactory).toRule());
