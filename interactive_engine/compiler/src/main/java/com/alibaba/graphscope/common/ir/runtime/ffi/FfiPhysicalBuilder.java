...
public class FfiPhysicalBuilder extends RegularPhysicalBuilder<Pointer, byte[]> {
    ...
    public FfiPhysicalBuilder(Configs graphConfig, IrMeta irMeta, LogicalPlan logicalPlan, PlanPointer planPointer) {
    ...
    public void appendNode(PhysicalNode<Pointer> node) {
        ...
        if (original instanceof GraphLogicalSource) {
            ...
            FfiPhysicalBuilder leftBuilder = new FfiPhysicalBuilder(graphConfig, irMeta, leftPlan, new PlanPointer(this.planPointer.ptrPlan));
            FfiPhysicalBuilder rightBuilder = new FfiPhysicalBuilder(graphConfig, irMeta, rightPlan, new PlanPointer(this.planPointer.ptrPlan));
            checkFfiResult(
    ...
    public byte[] build() {
        ...
        FfiData.ByValue ffiData =
                LIB.buildPhysicalPlan(this.planPointer.ptrPlan, getEngineWorkerNum(), getEngineServerNum());
    ...
    private void appendMatch(PhysicalNode<Pointer> node, IntByReference oprIdx) {
        ...
        checkFfiResult(LIB.appendScanOperator(this.planPointer.ptrPlan, ffiNodes.get(0), oprIdx.getValue(), oprIdx));
        checkFfiResult(
                LIB.appendPatternOperator(this.planPointer.ptrPlan, ffiNodes.get(1), oprIdx.getValue(), oprIdx));
    }
    ...
    private void appendProjectDedup(PhysicalNode<Pointer> node, IntByReference oprIdx) {
        ...
        checkFfiResult(
                LIB.appendProjectOperator(this.planPointer.ptrPlan, ffiNodes.get(0), oprIdx.getValue(), oprIdx));
        checkFfiResult(
                LIB.appendDedupOperator(this.planPointer.ptrPlan, ffiNodes.get(1), oprIdx.getValue(), oprIdx));
    }

    private void appendSink(IntByReference oprIdx) {
        Pointer ptrSink = LIB.initSinkOperator();
        checkFfiResult(LIB.appendSinkOperator(this.planPointer.ptrPlan, ptrSink, oprIdx.getValue(), oprIdx));
    }
