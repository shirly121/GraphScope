...
public class RexVariableAliasConverter extends RexVisitorImpl<RexNode> {
    ...
    public RexNode visitGraphVariable(RexGraphVariable variable) {
        ...
        String targetVarName = (delimPos < 0) ? targetAliasName : targetAliasName + AliasInference.DELIMITER + variable.getName().substring(delimPos + 1);
        return RexGraphVariable.of(targetAliasId, variable.getProperty(), targetVarName, variable.getType());
    }
}
